...
from docket import config, job_store, review_queue
from docket.logging_setup import configure, get_logger
        ...
        if suffix == ".pdf":
            with fitz.open(path) as document:
                if document.page_count > config.MAX_PDF_PAGES:
                    raise HTTPException(
                        413,
                        f"PDF has {document.page_count} pages; limit is {config.MAX_PDF_PAGES}",
                    )
        return path
    except Exception:
